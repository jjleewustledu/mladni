...
usage="$(basename "$0") [-h] [-d nmf_dataset] [-k mask_file] [-m memory_request]

where:  -h shows this help
        -d <nmf_dataset> specifies a subfolder of $WORK for NMF inference
	-k <mask_file> specifies a NIfTI mask to use during NMF inference
	-m <memory_request> provides requests from SLURM when using mask_file, default 32G"

outputDir=$WORK
maskFile=""
memoryRequest="32G"
while getopts ':hd:k:m:' option; do
    case "$option" in
	h) echo "$usage"
	   exit
	   ;;
...
scriptDir="/scratch/jjlee/Singularity/ADNI/VolBin"
...
do
    ...
    if [ -z $maskFile ]; then
	echo "no mask file, default memory request"
	...
	echo "memoryRequest -> ${memoryRequest}"
	${scriptDir}/create_submit_extractBasesMT_masked.sh ${inFiles} ${b} ${outputDir} ${scriptDir} ${maskFile} ${memoryRequest} > ${outputDir}/NumBases${b}/submit_extractBasesMT.sh	
    fi
