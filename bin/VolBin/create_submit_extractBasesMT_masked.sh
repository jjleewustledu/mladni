...
echo -n "command=\"\${BinDir}/run_extractBasesMT.sh \${MCRROOT} OPNMF \${inFiles} 1 \${numBases} outputDir \${outputDir} ${PARAMETERS[@]} mask \${maskFile}\""
echo -e "\n"
echo 'echo ${command} > ${outputDir}/NumBases${numBases}/command_${date2save}.txt  '

# running main command
echo "\${extractBases} \${MCRROOT} OPNMF \${inFiles} 1 \${numBases} outputDir \${outputDir} ${PARAMETERS[@]} mask \${maskFile}"
echo '  '
