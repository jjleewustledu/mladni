...
```{r dev=c("png", "tiff")}
metaroi <- tib_cohorts$Metaroi
cdglobal <- tib_cohorts$CDGLOBAL
tib_cdr <- tibble(metaroi, cdglobal, age, sex, apoe4, cohort)
tib_cdr$sex <- relevel(factor(tib_cdr$sex), ref="F")
tib_cdr$cohort <- relevel(factor(tib_cdr$cohort), ref="Normal")
tib_cdr
summary(tib_cdr)

# build gam
b_cdr <- gam(metaroi~s(cdglobal)+s(age)+sex+apoe4+cohort, data=tib_cdr)
b_cdr
summary(b_cdr)
plot(b_cdr, page=1)
AIC(b_cdr, k=2)
...
for (idx in 1:16) {
  ...
  # gather variables
  y_i <- Y[,idx]
  tib_y_i <- tibble(y_i, cdglobal, age, sex, apoe4, cohort)
  tib_y_i$sex <- relevel(factor(tib_y_i$sex), ref="F")
  tib_y_i$cohort <- relevel(factor(tib_y_i$cohort), ref="Normal")
  tib_y_i
  summary(tib_y_i)
  ...
  # build gam, univariate
  b_i <- gam(y_i~s(cdglobal)+s(age)+sex+apoe4+cohort, data=tib_y_i)
  #b_i
  #summary(b_i)
  #plot(b_i, page=1)
  AIC(b_i, k=2)
